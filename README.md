...
### Как это работает
Мод работает **универсально** - один и тот же код работает и на клиенте, и на сервере:

- **Обнаружение игроков** происходит по запросу через команды
- **Данные кэшируются** локально для каждого игрока
- **Статистика сохраняется** в JSON файлы
- **Команды доступны** всем игрокам без прав OP

### Преимущества универсального подхода
✅ **Простота** - один код работает везде  
✅ **Надежность** - нет сложных событий и синхронизации  
✅ **Производительность** - обнаружение только по запросу  
✅ **Совместимость** - работает на любом сервере с NeoForge  

### Команды для работы
```bash
/playertracker scan          # Принудительно сканировать игроков
/playertracker list          # Показать обнаруженных игроков
/playertracker cache clear   # Очистить кэш
/playertracker stats show    # Показать статистику
```
